...
namespace PMS.Libraries.ToolControls
{
	public class MESReportRun
	{
		...
		/*private void ClearQueryFiles ()
		{
			...
			}, (d) => d.ToRunEvery (1).Days ().At (0, 0)); // 每天00:00清理
		}*/
...
//					}
		...
		{
			var min = ConfigurationManager.AppSettings.Get ("removeQueryIdPeroid");
			var removeQueryIdInterval = ConfigurationManager.AppSettings.Get ("removeQueryIdInterval");
			var minutes = 30;
			...
			var interval = 30;
			if (!int.TryParse (removeQueryIdInterval, out interval)) {
				interval = 30;
			}
			JobManager.AddJob (() => {
				try {
					...
					foreach (var qm in this.ReportQueryManager.QueryingReportDic) {
						if (DateTime.Now.AddMinutes (-1 * minutes).ToString ("yyyyMMddHHmmssffff").CompareTo (qm.Value.CreateTime) >= 0) {
							list.Add (qm.Key);
			...
			}, (m) => m.ToRunEvery (interval).Minutes ());
		}
		...
		/// <returns></returns>
		public bool RunReportServer (string fileFullPath, string rptID, string queryID, bool isExportMode, ref string filterStr, ReportViewType reportViewType = ReportViewType.None, bool isShowDialog = true, string exportFileName = "")
		{
			MESReportFileObj reportFileObj = new MESReportFileObj ();
			try {
				if (!DBFileManager.LoadReportFile (fileFullPath, ref reportFileObj))
				...
				StartRunReportServerThread (reportFileObj, rptID, queryID, isExportMode, ref filterStr, reportViewType, isShowDialog, exportFileName);
			} catch (System.Exception ex) {
				PMS.Libraries.ToolControls.PMSPublicInfo.Message.Error (ex.Message);
				return false;
			} finally {
		...
		public void StartRunReportServerThread (MESReportFileObj reportFileObj, string rptID, string queryID, bool isExportMode, ref string filterStr, ReportViewType reportViewType = ReportViewType.None, bool isShowDialog = true, string exportFileName = "")
		{
			try {
...
//				}
				_ReportServerQueryManager.Add (rptItem.QueryID,
					new QueryingReport () {
						...
						ExportFileName = rptItem.ExportFileName,
...
//				mainReportDrawing.ZoomMode = NetSCADA.ReportEngine.ZoomEnum.FitWidth;
				...
				mainReportDrawing.ExportFileName = qr.ExportFileName;
		...
		void mainReportDrawing_OnExportCompleteCallBack (object sender, NetSCADA.ReportEngine.ExportCompletedArgs e)
		{
			...
			QueryingReport qr = _ReportServerQueryManager.Get (queryid);
			PMS.Libraries.ToolControls.PMSPublicInfo.Message.Info (string.Format ("OnExportComplete,queryid:{0}", queryid));
			if (null != qr) {//todo:qiuleilei
                
				qr.IsCompleted = 1;
				PMS.Libraries.ToolControls.PMSPublicInfo.Message.Info (string.Format ("OnExportComplete,queryid:{1},IsCompleted={0}", qr.IsCompleted, queryid));
			}
