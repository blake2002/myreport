...
{
    public partial class FormLegendItemCollection : Form
    {
        public FormLegendItemCollection()
        {
            InitializeComponent();
        }
        private List<string> existName = new List<string>();
        private List<PMSLegendItem> dataList;
        public List<PMSLegendItem> DataList
        {
            get
            {
                if (dataList == null)
                    dataList = new List<PMSLegendItem>();
                dataList.Clear();

                foreach (var obj in listBox1.Items)
                {
                    dataList.Add((PMSLegendItem)obj);
                }
                return dataList;
            }
            set
        ...
        {
            PMSLegendItem pca = new PMSLegendItem(null);
            pca.Parent = existName;
            pca.Name = GetNewName("LegendItem");
        ...
        private string GetNewName(string primary)
        {
            for (int i = 0; i < listBox1.Items.Count; i++)
            {
                ...
                {
                    PMSLegendItem child1 = (PMSLegendItem)child;
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                if (listBox1.Items.Count <= 0)
                    return;
                PMSLegendItem pca = (PMSLegendItem)listBox1.SelectedItem;
        ...
        private void propertyGrid1_PropertyValueChanged(object s, PropertyValueChangedEventArgs e)
        {
            if (e.ChangedItem.Label.Equals("(Name)", StringComparison.CurrentCultureIgnoreCase))
            {
                ...
                List<PMSLegendItem> list = DataList;
                DataList = list;
