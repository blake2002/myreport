...
//				}
		...
		string GetExportPath (string queryID, ReportViewType reportViewType, string time, ref string url)
		{
			...
			if (string.IsNullOrEmpty (fileName))
				return string.Empty;
			string filehashcode = _ExportFileName;//Util.Hash_MD5_16 (fileName, false);
			...
			case ReportViewType.ExportQuery:
				...
				url = string.Format ("{0}/{1}/{2}", ProjectPathClass.MesReportServerServerWeb, ProjectPathClass.MesReportServerExport, filehashcode.Replace (Path.DirectorySeparatorChar, '/'));
				break;
			case ReportViewType.Query:
				exportDir = ProjectPathClass.ServerWebQuery;
				filehashcode = Util.Hash_MD5_16 (fileName, false);
				var queryFile = string.Format ("{0}_{1}", queryID, filehashcode);
