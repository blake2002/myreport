...
{
    public partial class FormLegendCellCollection : Form
    {
        public FormLegendCellCollection()
        {
            InitializeComponent();
        }
        private List<string> existName = new List<string>();
        private List<PMSLegendCell> dataList;
        public List<PMSLegendCell> DataList
        {
            get
            {
                if (dataList == null)
                    dataList = new List<PMSLegendCell>();
                dataList.Clear();

                foreach (var obj in listBox1.Items)
                {
                    dataList.Add((PMSLegendCell)obj);
                }
                return dataList;
            }
            set
        ...
        {
            PMSLegendCell pca = new PMSLegendCell(null);            
            pca.Parent = existName;
            pca.Name = GetNewName("LegendCell");
        ...
        private string GetNewName(string primary)
        {
            for (int i = 0; i < listBox1.Items.Count; i++)
            {
                ...
                {
                    PMSLegendCell child1 = (PMSLegendCell)child;
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                if (listBox1.Items.Count <= 0)
                    return;
                PMSLegendCell pca = (PMSLegendCell)listBox1.SelectedItem;
        ...
        private void propertyGrid1_PropertyValueChanged(object s, PropertyValueChangedEventArgs e)
        {
            if (e.ChangedItem.Label.Equals("Name", StringComparison.CurrentCultureIgnoreCase))
            {
                ...
                List<PMSLegendCell> list = DataList;
                DataList = list;
