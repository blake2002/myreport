...
namespace NetSCADA.ReportEngine
{
	public static class External
	{
		...
		{
//			if (tag.HasClass (CssDefault.DefaultCssStyleName) && defaultStyle.Exists (styleName.Trim (), styleVal.Trim ()))
//				return tag;
	...
	public class CssDefault
	{
		...
		public const string DefaultCssStyleName = "element";
		...
		private CssDefault ()
		{
			...
			cssClassDict.Add (DefaultCssStyleName, defalutStyle);
		...
		public bool Exists (string styleName, string styleVal, string className = DefaultCssStyleName)
		{
		...
		public void ToCss (string path)
		{
			var file = Path.Combine (path, "main.css");
			if (File.Exists (file))
				return;
			...
			using (var css = File.Create (file)) {
				using (var sw = new StreamWriter (css)) {
					sw.Write (result);
					sw.Flush ();
				}	
			}
		...
		public void Release ()
		{
			if (cssClassDict != null && cssClassDict.Count != 0) {
				cssClassDict.Clear ();
			}
		}
	}
	...
	{
		static readonly string HtmlDoc = @"<!DOCTYPE html><html><head><meta charset=""UTF-8""><title>{0}</title><script src=""../../jquery-1.10.2.min.js""></script><script src=""../../echarts.min.js""></script></head><body>{1}</body></html>";
		...
		public CssDefault Css { 
			get { return cssDefault; } 
			set { cssDefault = value; } 
		}
		...
		public string ToHtml (ReportPage page)
		{
			...
			if (HtmlDict.TryGetValue (page.PageNumber, out sb)) {
				...
				var result = sb.ToString ();
//				if (page.PageNumber == 1) {// 前端要求 只第一页 需要 html header body
//					result = string.Format (HtmlDoc, page.PageNumber, sb.ToString ());
//				}
				sb.Clear ();
				return result;
			} else {
...
//				.Style ("font-size", string.Format ("{0}px", element.Font.Size));
		...
		string GetImage (PmsImageBox img, string imgDirName)
		{
			var name = string.IsNullOrEmpty (img.Name) ? Guid.NewGuid ().ToString ("N") : img.Name;
			var fileName = string.Format ("{0}.png", name);
			var imgDir = Path.Combine (this.HtmlPath, imgDirName);
			if (!Directory.Exists (imgDir)) {
				Directory.CreateDirectory (imgDir);
			}
			var imgFileName = string.Format ("{0}{1}{2}", imgDir, Path.DirectorySeparatorChar, fileName);
			if (!File.Exists (imgFileName)) {
				img.Image.Save (imgFileName, ImageFormat.Png);
			}
			string url = string.Format ("url({0}/{1})", imgDirName, fileName);
			return url;
		}

		string GetImage (Image img, string imgDirName)
		{
...
//						}
		...
		{
			if (!(isExport && CurrentPrjInfo.CurrentEnvironment == MESEnvironment.MESReportServer))
				return;
			var tag = tagPool.Get ();//new DivTag ();
			tag.Dispose ();// 清理数据
			string postion = "absolute";
//			if (page.FirstPosition == 1) {
//				postion = "relative";
//				page.FirstPosition = -1;
//			}
			tag.StyleEx ("position", postion, cssDefault)
...
//			}
		...
		public void CurveChart2Html (ReportPage page, NetSCADACurve curve, PointF location, float width, float height, bool isClosed = true)
		{
			...
			tag.Id (curveName);
			tag.StyleEx ("position", "absolute", cssDefault)
				.StyleEx ("width", string.Format ("{0}px", width), cssDefault)
				.StyleEx ("height", string.Format ("{0}px", height), cssDefault)
				.StyleEx ("top", string.Format ("{0}px", location.Y), cssDefault)
				.StyleEx ("left", string.Format ("{0}px", location.X), cssDefault);
...
//			var title = new Title ();
			...
			if (flag == 0) {
				...
				sb.Append (@"function(p,ticket,callback){var result=[];");
				sb.Append (string.Format (" seriesNames=[{0}]", features));
				sb.Append (@";p.forEach(function(e){if(seriesNames.indexOf(e.seriesName)==-1)");
				sb.Append (@"{if(e.value != undefined){var a='<li>");
				sb.Append (@"<span style=""color:'+e.color+'"">●</span>");
				sb.Append (@"'+e.seriesName+':'+e.value.toString()+'</li>'");
				sb.Append (@";result.push(a)}}});if(result.length!=0){var");
				sb.Append (@"r='<ul style=""margin:0;padding:0;color:#ddd;");
				sb.Append (@"font-size:15px;list-style-type: none"">'");
				sb.Append (@"+result.join('')+'</ul>';return r}else return''}");
				option.ToolTip ().Trigger (TriggerType.axis).Formatter (new JRaw (sb.ToString ()));
				sb.Clear ();
				sb = null;
			}
...
//				}
		...
		TextStyle GetTextStyle (Title title, PMSTitle titleApperence)
		{
			...
			if (cssDefault != null && !cssDefault.Exists ("font-family", titleApperence.Font.FontFamily.Name))
				txtStyle.FontFamily (titleApperence.Font.FontFamily.Name);
			if (cssDefault != null && !cssDefault.Exists ("font-size", FontSize2Px (titleApperence.Font.Size).ToString ()))
