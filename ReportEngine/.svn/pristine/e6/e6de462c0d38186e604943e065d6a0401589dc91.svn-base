...
{
    public partial class FormCustomLabelCollection : Form
    {
        public FormCustomLabelCollection()
        {
            InitializeComponent();
        }
        private List<string> existName = new List<string>();
        private List<PMSCustomLabel> dataList;
        public List<PMSCustomLabel> DataList
        {
            get
            {
                if (dataList == null)
                    dataList = new List<PMSCustomLabel>();
                dataList.Clear();

                foreach (var obj in listBox1.Items)
                {
                    dataList.Add((PMSCustomLabel)obj);
                }
                return dataList;
            }
            set
        ...
        {
            PMSCustomLabel pca = new PMSCustomLabel(null);            
            pca.Parent = existName;
            pca.Name = GetNewName("CustomLabel");
        ...
        private string GetNewName(string primary)
        {
            for (int i = 0; i < listBox1.Items.Count; i++)
            {
                ...
                {
                    PMSCustomLabel child1 = (PMSCustomLabel)child;
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                if (listBox1.Items.Count <= 0)
                    return;
                PMSCustomLabel pca = (PMSCustomLabel)listBox1.SelectedItem;
        ...
        private void propertyGrid1_PropertyValueChanged(object s, PropertyValueChangedEventArgs e)
        {
            if (e.ChangedItem.Label.Equals("Name", StringComparison.CurrentCultureIgnoreCase))
            {
                ...
                List<PMSCustomLabel> list = DataList;
                DataList = list;
