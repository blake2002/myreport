...
{
    public partial class FormLegendCellColumnCollection : Form
    {
        public FormLegendCellColumnCollection()
        {
            InitializeComponent();
        }
        private List<string> existName = new List<string>();
        private List<PMSLegendCellColumn> dataList;
        public List<PMSLegendCellColumn> DataList
        {
            get
            {
                if (dataList == null)
                    dataList = new List<PMSLegendCellColumn>();
                dataList.Clear();

                foreach (var obj in listBox1.Items)
                {
                    dataList.Add((PMSLegendCellColumn)obj);
                }
                return dataList;
            }
            set
        ...
        {
            PMSLegendCellColumn pca = new PMSLegendCellColumn(null);
            pca.Parent = existName;
            pca.Name = GetNewName("column");
        ...
        private string GetNewName(string primary)
        {
            for (int i = 0; i < listBox1.Items.Count; i++)
            {
                ...
                {
                    PMSLegendCellColumn child1 = (PMSLegendCellColumn)child;
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                if (listBox1.Items.Count <= 0)
                    return;
                PMSLegendCellColumn pca = (PMSLegendCellColumn)listBox1.SelectedItem;
        ...
        private void propertyGrid1_PropertyValueChanged(object s, PropertyValueChangedEventArgs e)
        {
            if (e.ChangedItem.Label.Equals("Name", StringComparison.CurrentCultureIgnoreCase))
            {
                ...
                List<PMSLegendCellColumn> list = DataList;
                DataList = list;
