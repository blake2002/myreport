...
namespace NetSCADA.ReportEngine
{
		...
		private CssDefault ()
		{
			...
			var defalutStyle = new Dictionary<string, string> () {
				...
				{ "border-width", "1px" }, {
					...
					"font-size",
					string.Format ("{0}px", Extension.FontSize2Px (SystemFonts.DefaultFont.Size))
				}
			};
	...
	{
		static readonly string HtmlDoc = @"<!DOCTYPE html>
<html>
	<head>
		<meta charset=""UTF-8"">
		<title>{0}</title>
	...
	<body>
		...
		static string[] CustomKeys = new string[] {
			"#LABEL",
			"#LEGENDTEXT",
			"#AXISLABEL",
			"#SERIESNAME",
		...
		static readonly CssDefault cssDefault = CssDefault.Instance ();

		public static string ToHtml (ReportPage page)
		{
		...
		//todo: qiuleilei
		static Dictionary<int,StringBuilder> HtmlDict = new Dictionary<int, StringBuilder> ();
		static Dictionary<int,StringBuilder> JavaScriptDict = new Dictionary<int, StringBuilder> ();

		public static void CleartHtml ()
		{
			if (HtmlDict != null) {
				foreach (var key in HtmlDict.Keys) {
					HtmlDict [key].Clear ();
					//HtmlDict [key] = null;
				}
				HtmlDict.Clear ();
			...
			if (JavaScriptDict != null) {
				foreach (var key in JavaScriptDict.Keys) {
					JavaScriptDict [key].Clear ();
					//JavaScriptDict [key] = null;
				}
		...
		public static int FontSize2Px (float fontSize)
		{
			return (int)System.Math.Floor (fontSize / 72 * 96);
		}

		public static void Cell2Html (ReportPage page, ICell cell, PointF location)
		{
			if (cell == null)
				return;
			var tag = new DivTag ();
...
//				.Style ("font-size", string.Format ("{0}px", cell.Style.Font.Size));
			...
			tag	
				.StyleEx ("font-family", string.Format ("\'{0}\'", cell.Style.Font.FontFamily.Name), cssDefault)
		...
		public static HtmlTag Element2Html (ReportPage page, IElement element, PointF location, float width, float height, bool isClosed = true)
		{
			if (element == null)
				return null;
...
//				.Style ("font-size", string.Format ("{0}px", element.Font.Size));
		...
		public static void Label2Html (ReportPage page, PmsLabel label, PointF location, float width, float height)
		{
		...
		public static void Image2Html (ReportPage page, PmsImageBox img, PointF location, float Width, float height)
		{
		...
		public static void BarCode2Html (ReportPage page, BarCode barCode, PointF location, float Width, float height)
		{
		...
		public static void QrCode2Html (ReportPage page, QRCode qrCode, PointF location, float Width, float height)
		{
		...
		public static void ReportPageType2Html (ReportPage page, RectangleF rect, Color color, bool isExport)
		{
		...
		public static void ToHtml (ReportPage page, ReportRuntime rptRuntime, object obj, PointF location, float Width, float height)
		{
...
//			}
		...
		public static void CurveChart2Html (ReportPage page, NetSCADACurve curve, PointF location, float width, float height, bool isClosed = true)
		{
			if (curve == null)
				return;
			...
			Dictionary<string,object> xyDataDict = new Dictionary<string, object> ();
			...
			foreach (Curve c in datumCurves) {
				if (!xyDataDict.ContainsKey (c.Name)) {
					...
					xyDataDict.Add (c.Name, new {Y = c.strCoordY,
						LineColor = ColorTranslator.ToHtml (Color.FromArgb (c.Line.Color.R, c.Line.Color.G, c.Line.Color.B)),
						LineStyle = c.Line.Style,
						LineWidth = c.Line.Width,
						Data = dataList});
			...
			foreach (var f in curve.AryFeatureTags) {
				...
				if (!xyDataDict.ContainsKey (ftag.Name)) {
					...
					xyDataDict.Add (ftag.Name, new {Y = max.FirstOrDefault ().Key,
		...
		static void SetDataZoom (ChartOption option)
		{
			// x 轴缩放
		...
		static string GetCurveChartOption (NetSCADACurve curve, List<string> xData, Dictionary<string,object> xyDataDict, Dictionary<string,List<string[]>> featureTagDict)
		{
...
//					splitArea = new SplitArea (){ show = true },
			...
			foreach (var kv in xyDataDict) {
				var da = ChangeType (kv.Value, new {Y = "", LineColor = "", LineStyle = 0, LineWidth = 1,	Data = new List<object[]> ()});
		...
		public static void BarChart2Html (ReportPage page, DataTableManager dm, BarChart barChart, PointF location, float width, float height, bool isClosed = true)
		{
			if (barChart == null)
				return;
...
//				}
		...
		public static void PieChart2Html (ReportPage page, DataTableManager dm, PieChart pieChart, PointF location, float width, float height, bool isClosed = true)
		{
			if (pieChart == null)
				return;
...
//				}
		...
		static TextStyle GetTextStyle (Title title, PMSTitle titleApperence)
		{
		...
		static string GetPieChartOption (DivTag divTag, PieChart pieChart, Dictionary<string,List<string>> datas, Dictionary<string,Tuple<string,int>> Ys, Dictionary<string,string> lendDict, Dictionary<string,string> seriesColorDict, Dictionary<string,Dictionary<string,string>> fieldNameDict)
		{
			...
			foreach (var se in seList) {
				...
				for (var i = 0; i < xdata.Count; i++) {
					list.Add (new {name = xdata [i].ToString (),value = datas [se.Name] [i].ToString (),selected = false,itemStyle = default(object)});
				}
				...
				if (pieChart.ChartType == PieChartType.Pie) {
					b.Radius ("80%").Center (new List<string> (){ "25%", "55%" });
					...
					b.Radius (new List<string> (){ "80%", "15%" }).Center (new List<string> () {
						"25%",
						"55%"
					});
		...
		static void GetGridStyle (DivTag divTag, PMSChartArea area)
		{
...
//			}
		...
		static LineStyleType getLineStyleType (System.Windows.Forms.DataVisualization.Charting.ChartDashStyle type)
		{
		...
		static LineStyleType getLineStyleType (int style)
		{
		...
		static bool CollectedSlice (string serieName, List<object> list, List<string> xData, Dictionary<string,Dictionary<string,string>> fieldNameDict)
		{
			try {
				...
				if (int.TryParse (fieldNameDict [serieName] ["CollectedColor"], out collectColor)) {
					...
					list.Add (new {name = fieldNameDict [serieName] ["CollectedLegendText"],value = lessSum.ToString (),selected = true,itemStyle = new {normal = new {color = cc}}});
				} else {
					list.Add (new {name = fieldNameDict [serieName] ["CollectedLegendText"],value = lessSum.ToString (),selected = true,itemStyle = default(object)});
				}
		...
		static void SetOptionColor (ChartOption option)
		{
		...
		static string GetBarChartOption (BarChart barChart, Dictionary<string,List<string>> datas, Dictionary<string,Tuple<string,int,double,double>> Ys, Dictionary<string,string> lendDict, Dictionary<string,string> seriesColorDict, Dictionary<string,Dictionary<string,string>> fieldNameDict)
		{
			if (datas == null || datas.Count == 0)
				return "{}";
			var option = new ChartOption ();
...
//					splitArea = new SplitArea (){ show = true },
		...
		static Dictionary<string,List<object>> GetPercent (Dictionary<string,List<string>> datas)
		{
			// 计算系列个百分比
			var ydatas = datas.Where ((s) => s.Key != "x").ToDictionary ((kv) => kv.Key, (kv) => kv.Value);
			var percentDict = new Dictionary<string,List<object>> ();
		...
		static void AppendToHtmlStr (ReportPage page, HtmlTag tag)
		{
			StringBuilder sb = null;
			if (HtmlDict.TryGetValue (page.PageNumber, out sb)) {
				sb.AppendLine (tag.ToHtmlString ());
		...
		static void AppendToJavaScriptStr (ReportPage page, string script)
		{
			StringBuilder sb = null;
		...
		public static string ReplacePieKeywords (string strOriginal, List<string> xDatas, Dictionary<string,List<string>> datas)
		{
		...
		public static string ReplaceKeywords (string strOriginal, List<string> xDatas, Dictionary<string,List<string>> datas)
		{
		...
		public static string ReplaceKeywords (string strOriginal, string seriesName,
